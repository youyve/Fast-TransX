...
then
    ...
    echo "bash scripts/run_eval_npu.sh [DATASET_ROOT] [DATASET_NAME] [MODEL_NAME] [CKPT_PATH]"
    exit 1
fi
...
LOGS_DIR="eval-logs"
LOG_FILE="./$LOGS_DIR/eval_${MODEL_NAME}_${DATASET_NAME}.log"
...
# Run evaluation
python eval.py --config_path="$CONFIG_FILE" --dataset_root="$DATASET_ROOT" --ckpt_file="$CKPT_PATH"  > "$LOG_FILE" 2>&1 &
